...
class MusicService : Service() {
    ...
    fun loadConvolutionIr(
        ...
        val engine = convolutionEngine ?: run {
            onComplete?.invoke(false, "Engine não inicializado")
            return
        }
        Thread {
            IrLoader.loadIntoSlot(file, slot, engine) { success, error ->
                if (success) {
                    prefs.edit().putString("ir_slot_${slot.name}", file.absolutePath).apply()
                }
    ...
    private fun restoreConvolutionIrs() {
        val engine = convolutionEngine ?: return
        Thread {
            ConvolutionEngine.IrSlot.values().forEach { slot ->
                ...
                if (file.exists()) {
                    IrLoader.loadIntoSlot(file, slot, engine) { success, _ ->
                        Log.d("MusicService", "Restaurado IR $slot: $success")
                    }
                } else {
                    prefs.edit().remove("ir_slot_${slot.name}").apply()
                }
            }
            val wasEnabled = prefs.getBoolean("binaural_enabled", false)
            if (wasEnabled) engine.enabled = true
        }.apply { isDaemon = true; start() }
    }
        ...
        player = StreamingAudioPlayer().apply {
            ...
            onPrepared = {
                handler.post {
                    ...
                    // Carrega IRs padrão (assets) se nenhum slot estiver carregado
                    val engine = player?.convolutionEngine
                    if (engine != null && !engine.hasPrincipalIrs()) {
                        Thread {
                            IrLoader.loadAllDefaults(applicationContext, engine) { slot, success, error ->
                                if (success) Log.d("MusicService", "IR padrão: $slot")
                                else Log.w("MusicService", "IR $slot falhou: $error")
                            }
