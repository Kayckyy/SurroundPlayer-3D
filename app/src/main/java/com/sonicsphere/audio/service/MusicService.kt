...
class MusicService : Service() {
    ...
    // ========== BINAURAL (ConvolutionEngine) ==========

    fun isBinauralEnabled(): Boolean =
        player?.convolutionEngine?.enabled ?: false

    fun setBinauralEnabled(enabled: Boolean) {
        player?.convolutionEngine?.enabled = enabled
        prefs.edit().putBoolean("binaural_enabled", enabled).apply()
        Log.d("MusicService", "🎧 Binaural ${if (enabled) "ativado" else "desativado"}")
    }

    fun getConvolutionEngine() = player?.convolutionEngine
    ...
    fun playMusic(index: Int) {
        ...
        Log.d("MusicService", "   Haas: ${savedHaasDelay}ms, Pitch: $savedPitch, Speed: ${savedSpeed}x")
    ...
    fun seekTo(position: Int) {
        try {
            if (isPrepared && player != null) {
                // StreamingAudioPlayer.seekTo já retoma o play internamente se necessário.
                // Não chamamos player?.play() aqui para evitar o "click" de dupla retomada.
