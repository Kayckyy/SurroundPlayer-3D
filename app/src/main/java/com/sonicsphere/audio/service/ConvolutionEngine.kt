...
class ConvolutionEngine(private val sampleRate: Int) {
    ...
    private fun processBlock() {
        ...
        // Resultado acumulado (complexo)
        val resL = FloatArray(FFT_SIZE * 2)
        val resR = FloatArray(FFT_SIZE * 2)

        // Principais
        irFreq[IrSlot.LEFT]?.let  { complexMulAdd(fL,   it, resL) }
        irFreq[IrSlot.RIGHT]?.let { complexMulAdd(fR,   it, resR) }

        // Reflexões
        irFreq[IrSlot.FRONT]?.let { irF ->
            val ref = FloatArray(FFT_SIZE * 2)
            complexMulAdd(fMid, irF, ref)
            val t = toTime(ref)
            // Delay assimétrico L=0.3ms, R=0.7ms
            addWithDelay(t, resL, 13, GAIN_FRONT, invert = false)
            addWithDelay(t, resR, 31, GAIN_FRONT, invert = false)
        }
        ...
        irFreq[IrSlot.TOP]?.let { irF ->
            ...
            for (i in t.indices) {
                resL[i * 2] += t[i] * GAIN_TOP
                resR[i * 2] += tDecorr[i] * GAIN_TOP
            }
        }
        ...
        irFreq[IrSlot.BACK]?.let { irF ->
            ...
            addWithDelay(t, resL, 66, GAIN_BACK, invert = false)
            addWithDelay(t, resR,  0, GAIN_BACK, invert = true)
        }
        ...
        irFreq[IrSlot.SUB]?.let { irF ->
            ...
            for (i in t.indices) {
                ...
                resL[i * 2] += subLpfL.toFloat() * GAIN_SUB
                resR[i * 2] += subLpfR.toFloat() * GAIN_SUB
            }
        }
    ...
    private fun addWithDelay(src: FloatArray, dst: FloatArray, delay: Int, gain: Float, invert: Boolean) {
        val sign = if (invert) -gain else gain
        for (i in src.indices) {
            val srcIdx = (i - delay).coerceAtLeast(0)
            dst[i * 2] += src[srcIdx] * sign
        }
    }
