...
class ConvolutionEngine(private val sampleRate: Int) {
    ...
    private val irRmsGain   = mutableMapOf<IrSlot, Float>()  // fator de compensação por slot
    ...
    fun loadIr(slot: IrSlot, leftChannel: FloatArray, rightChannel: FloatArray) {
        ...
        // Normalização por energia RMS — preserva o caráter tonal do IR
        // sem permitir acumulação de energia que causa clipping
        val rms = sqrt(ir.map { it * it }.average()).toFloat()
        val gain = if (rms > 1e-6f) {
            // Escala para que RMS da saída ≈ RMS da entrada
            // Fator conservador: 0.5 para deixar headroom
            0.5f / rms
        } else {
            1.0f
        }
        ...
        reset()
        Log.d(TAG, "IR carregado: $slot | $len samples | RMS=$rms | gain=$gain")
    }

    fun unloadIr(slot: IrSlot) {
        irFreq.remove(slot)
        irRmsGain.remove(slot)
